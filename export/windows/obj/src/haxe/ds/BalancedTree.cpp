// Generated by Haxe 4.0.0-rc.2+77068e10c
#include <hxcpp.h>

#ifndef INCLUDED_Reflect
#include <Reflect.h>
#endif
#ifndef INCLUDED_haxe_IMap
#include <haxe/IMap.h>
#endif
#ifndef INCLUDED_haxe_ds_BalancedTree
#include <haxe/ds/BalancedTree.h>
#endif
#ifndef INCLUDED_haxe_ds_TreeNode
#include <haxe/ds/TreeNode.h>
#endif

HX_DEFINE_STACK_FRAME(_hx_pos_97671fefbd3e0550_41_new,"haxe.ds.BalancedTree","new",0xfb50b0cf,"haxe.ds.BalancedTree.new","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",41,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_51_set,"haxe.ds.BalancedTree","set",0xfb547c11,"haxe.ds.BalancedTree.set","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",51,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_61_get,"haxe.ds.BalancedTree","get",0xfb4b6105,"haxe.ds.BalancedTree.get","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",61,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_83_remove,"haxe.ds.BalancedTree","remove",0x68366b35,"haxe.ds.BalancedTree.remove","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",83,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_99_exists,"haxe.ds.BalancedTree","exists",0x238deccd,"haxe.ds.BalancedTree.exists","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",99,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_145_setLoop,"haxe.ds.BalancedTree","setLoop",0xe1a32b15,"haxe.ds.BalancedTree.setLoop","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",145,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_158_removeLoop,"haxe.ds.BalancedTree","removeLoop",0x52157c39,"haxe.ds.BalancedTree.removeLoop","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",158,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_182_merge,"haxe.ds.BalancedTree","merge",0x620fbce7,"haxe.ds.BalancedTree.merge","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",182,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_190_minBinding,"haxe.ds.BalancedTree","minBinding",0xd2237344,"haxe.ds.BalancedTree.minBinding","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",190,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_196_removeMinBinding,"haxe.ds.BalancedTree","removeMinBinding",0xe82c3b68,"haxe.ds.BalancedTree.removeMinBinding","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",196,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_200_balance,"haxe.ds.BalancedTree","balance",0xf7f4e10b,"haxe.ds.BalancedTree.balance","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",200,0x9022da28)
HX_LOCAL_STACK_FRAME(_hx_pos_97671fefbd3e0550_215_compare,"haxe.ds.BalancedTree","compare",0x57d05c94,"haxe.ds.BalancedTree.compare","C:\\HaxeToolkit\\haxe\\std/haxe/ds/BalancedTree.hx",215,0x9022da28)
namespace haxe{
namespace ds{

void BalancedTree_obj::__construct(){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_41_new)
            	}

Dynamic BalancedTree_obj::__CreateEmpty() { return new BalancedTree_obj; }

void *BalancedTree_obj::_hx_vtable = 0;

Dynamic BalancedTree_obj::__Create(hx::DynamicArray inArgs)
{
	hx::ObjectPtr< BalancedTree_obj > _hx_result = new BalancedTree_obj();
	_hx_result->__construct();
	return _hx_result;
}

bool BalancedTree_obj::_hx_isInstanceOf(int inClassId) {
	return inClassId==(int)0x00000001 || inClassId==(int)0x04042deb;
}

static ::haxe::IMap_obj _hx_haxe_ds_BalancedTree__hx_haxe_IMap= {
	(  ::Dynamic (hx::Object::*)( ::Dynamic))&::haxe::ds::BalancedTree_obj::get,
	( void (hx::Object::*)( ::Dynamic, ::Dynamic))&::haxe::ds::BalancedTree_obj::set_41d0cb46,
	( bool (hx::Object::*)( ::Dynamic))&::haxe::ds::BalancedTree_obj::exists,
	( bool (hx::Object::*)( ::Dynamic))&::haxe::ds::BalancedTree_obj::remove,
};

void BalancedTree_obj::set_41d0cb46( ::Dynamic k, ::Dynamic v) {
			set(k,v);
}
void *BalancedTree_obj::_hx_getInterface(int inHash) {
	switch(inHash) {
		case (int)0x09c2bd39: return &_hx_haxe_ds_BalancedTree__hx_haxe_IMap;
	}
	#ifdef HXCPP_SCRIPTABLE
	return super::_hx_getInterface(inHash);
	#else
	return 0;
	#endif
}

void BalancedTree_obj::set( ::Dynamic key, ::Dynamic value){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_51_set)
HXDLIN(  51)		this->root = this->setLoop(key,value,this->root);
            	}


HX_DEFINE_DYNAMIC_FUNC2(BalancedTree_obj,set,(void))

 ::Dynamic BalancedTree_obj::get( ::Dynamic key){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_61_get)
HXLINE(  62)		 ::haxe::ds::TreeNode node = this->root;
HXLINE(  63)		while(hx::IsNotNull( node )){
HXLINE(  64)			int c = this->compare(key,node->key);
HXLINE(  65)			if ((c == 0)) {
HXLINE(  65)				return node->value;
            			}
HXLINE(  66)			if ((c < 0)) {
HXLINE(  66)				node = node->left;
            			}
            			else {
HXLINE(  67)				node = node->right;
            			}
            		}
HXLINE(  69)		return null();
            	}


HX_DEFINE_DYNAMIC_FUNC1(BalancedTree_obj,get,return )

bool BalancedTree_obj::remove( ::Dynamic key){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_83_remove)
HXDLIN(  83)		try {
            			HX_STACK_CATCHABLE(::String, 0);
HXLINE(  84)			this->root = this->removeLoop(key,this->root);
HXLINE(  85)			return true;
            		} catch( ::Dynamic _hx_e) {
            			if (_hx_e.IsClass< ::String >() && _hx_e->toString()!=null() ){
            				HX_STACK_BEGIN_CATCH
            				::String e = _hx_e;
HXLINE(  88)				return false;
            			}
            			else {
            				HX_STACK_DO_THROW(_hx_e);
            			}
            		}
HXLINE(  83)		return false;
            	}


HX_DEFINE_DYNAMIC_FUNC1(BalancedTree_obj,remove,return )

bool BalancedTree_obj::exists( ::Dynamic key){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_99_exists)
HXLINE( 100)		 ::haxe::ds::TreeNode node = this->root;
HXLINE( 101)		while(hx::IsNotNull( node )){
HXLINE( 102)			int c = this->compare(key,node->key);
HXLINE( 103)			if ((c == 0)) {
HXLINE( 103)				return true;
            			}
            			else {
HXLINE( 104)				if ((c < 0)) {
HXLINE( 104)					node = node->left;
            				}
            				else {
HXLINE( 105)					node = node->right;
            				}
            			}
            		}
HXLINE( 107)		return false;
            	}


HX_DEFINE_DYNAMIC_FUNC1(BalancedTree_obj,exists,return )

 ::haxe::ds::TreeNode BalancedTree_obj::setLoop( ::Dynamic k, ::Dynamic v, ::haxe::ds::TreeNode node){
            	HX_GC_STACKFRAME(&_hx_pos_97671fefbd3e0550_145_setLoop)
HXLINE( 146)		if (hx::IsNull( node )) {
HXLINE( 146)			return  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,null(),k,v,null(),null());
            		}
HXLINE( 147)		int c = this->compare(k,node->key);
HXLINE( 148)		if ((c == 0)) {
HXLINE( 148)			int _hx_tmp;
HXDLIN( 148)			if (hx::IsNull( node )) {
HXLINE( 148)				_hx_tmp = 0;
            			}
            			else {
HXLINE( 148)				_hx_tmp = node->_height;
            			}
HXDLIN( 148)			return  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,node->left,k,v,node->right,_hx_tmp);
            		}
            		else {
HXLINE( 149)			if ((c < 0)) {
HXLINE( 150)				 ::haxe::ds::TreeNode nl = this->setLoop(k,v,node->left);
HXLINE( 151)				return this->balance(nl,node->key,node->value,node->right);
            			}
            			else {
HXLINE( 153)				 ::haxe::ds::TreeNode nr = this->setLoop(k,v,node->right);
HXLINE( 154)				return this->balance(node->left,node->key,node->value,nr);
            			}
            		}
HXLINE( 148)		return null();
            	}


HX_DEFINE_DYNAMIC_FUNC3(BalancedTree_obj,setLoop,return )

 ::haxe::ds::TreeNode BalancedTree_obj::removeLoop( ::Dynamic k, ::haxe::ds::TreeNode node){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_158_removeLoop)
HXLINE( 159)		if (hx::IsNull( node )) {
HXLINE( 159)			HX_STACK_DO_THROW(HX_("Not_found",b6,92,a5,ea));
            		}
HXLINE( 160)		int c = this->compare(k,node->key);
HXLINE( 161)		if ((c == 0)) {
HXLINE( 161)			return this->merge(node->left,node->right);
            		}
            		else {
HXLINE( 162)			if ((c < 0)) {
HXLINE( 162)				 ::haxe::ds::TreeNode _hx_tmp = this->removeLoop(k,node->left);
HXDLIN( 162)				return this->balance(_hx_tmp,node->key,node->value,node->right);
            			}
            			else {
HXLINE( 163)				 ::haxe::ds::TreeNode node1 = node->left;
HXDLIN( 163)				 ::Dynamic node2 = node->key;
HXDLIN( 163)				 ::Dynamic node3 = node->value;
HXDLIN( 163)				return this->balance(node1,node2,node3,this->removeLoop(k,node->right));
            			}
            		}
HXLINE( 161)		return null();
            	}


HX_DEFINE_DYNAMIC_FUNC2(BalancedTree_obj,removeLoop,return )

 ::haxe::ds::TreeNode BalancedTree_obj::merge( ::haxe::ds::TreeNode t1, ::haxe::ds::TreeNode t2){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_182_merge)
HXLINE( 183)		if (hx::IsNull( t1 )) {
HXLINE( 183)			return t2;
            		}
HXLINE( 184)		if (hx::IsNull( t2 )) {
HXLINE( 184)			return t1;
            		}
HXLINE( 185)		 ::haxe::ds::TreeNode t = this->minBinding(t2);
HXLINE( 186)		 ::Dynamic t3 = t->key;
HXDLIN( 186)		 ::Dynamic t4 = t->value;
HXDLIN( 186)		return this->balance(t1,t3,t4,this->removeMinBinding(t2));
            	}


HX_DEFINE_DYNAMIC_FUNC2(BalancedTree_obj,merge,return )

 ::haxe::ds::TreeNode BalancedTree_obj::minBinding( ::haxe::ds::TreeNode t){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_190_minBinding)
HXDLIN( 190)		if (hx::IsNull( t )) {
HXDLIN( 190)			HX_STACK_DO_THROW(HX_("Not_found",b6,92,a5,ea));
            		}
            		else {
HXLINE( 191)			if (hx::IsNull( t->left )) {
HXLINE( 191)				return t;
            			}
            			else {
HXLINE( 192)				return this->minBinding(t->left);
            			}
            		}
HXLINE( 190)		return null();
            	}


HX_DEFINE_DYNAMIC_FUNC1(BalancedTree_obj,minBinding,return )

 ::haxe::ds::TreeNode BalancedTree_obj::removeMinBinding( ::haxe::ds::TreeNode t){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_196_removeMinBinding)
HXDLIN( 196)		if (hx::IsNull( t->left )) {
HXDLIN( 196)			return t->right;
            		}
            		else {
HXLINE( 197)			 ::haxe::ds::TreeNode _hx_tmp = this->removeMinBinding(t->left);
HXDLIN( 197)			return this->balance(_hx_tmp,t->key,t->value,t->right);
            		}
HXLINE( 196)		return null();
            	}


HX_DEFINE_DYNAMIC_FUNC1(BalancedTree_obj,removeMinBinding,return )

 ::haxe::ds::TreeNode BalancedTree_obj::balance( ::haxe::ds::TreeNode l, ::Dynamic k, ::Dynamic v, ::haxe::ds::TreeNode r){
            	HX_GC_STACKFRAME(&_hx_pos_97671fefbd3e0550_200_balance)
HXLINE( 201)		int hl;
HXDLIN( 201)		if (hx::IsNull( l )) {
HXLINE( 201)			hl = 0;
            		}
            		else {
HXLINE( 201)			hl = l->_height;
            		}
HXLINE( 202)		int hr;
HXDLIN( 202)		if (hx::IsNull( r )) {
HXLINE( 202)			hr = 0;
            		}
            		else {
HXLINE( 202)			hr = r->_height;
            		}
HXLINE( 203)		if ((hl > (hr + 2))) {
HXLINE( 204)			 ::haxe::ds::TreeNode _this = l->left;
HXDLIN( 204)			int _hx_tmp;
HXDLIN( 204)			if (hx::IsNull( _this )) {
HXLINE( 204)				_hx_tmp = 0;
            			}
            			else {
HXLINE( 204)				_hx_tmp = _this->_height;
            			}
HXDLIN( 204)			 ::haxe::ds::TreeNode _this1 = l->right;
HXDLIN( 204)			int _hx_tmp1;
HXDLIN( 204)			if (hx::IsNull( _this1 )) {
HXLINE( 204)				_hx_tmp1 = 0;
            			}
            			else {
HXLINE( 204)				_hx_tmp1 = _this1->_height;
            			}
HXDLIN( 204)			if ((_hx_tmp >= _hx_tmp1)) {
HXLINE( 204)				 ::haxe::ds::TreeNode l1 = l->left;
HXDLIN( 204)				 ::Dynamic l2 = l->key;
HXDLIN( 204)				 ::Dynamic l3 = l->value;
HXDLIN( 204)				return  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,l1,l2,l3, ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,l->right,k,v,r,null()),null());
            			}
            			else {
HXLINE( 205)				 ::haxe::ds::TreeNode _hx_tmp2 =  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,l->left,l->key,l->value,l->right->left,null());
HXDLIN( 205)				 ::Dynamic l4 = l->right->key;
HXDLIN( 205)				 ::Dynamic l5 = l->right->value;
HXDLIN( 205)				return  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,_hx_tmp2,l4,l5, ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,l->right->right,k,v,r,null()),null());
            			}
            		}
            		else {
HXLINE( 206)			if ((hr > (hl + 2))) {
HXLINE( 207)				 ::haxe::ds::TreeNode _this2 = r->right;
HXDLIN( 207)				int _hx_tmp3;
HXDLIN( 207)				if (hx::IsNull( _this2 )) {
HXLINE( 207)					_hx_tmp3 = 0;
            				}
            				else {
HXLINE( 207)					_hx_tmp3 = _this2->_height;
            				}
HXDLIN( 207)				 ::haxe::ds::TreeNode _this3 = r->left;
HXDLIN( 207)				int _hx_tmp4;
HXDLIN( 207)				if (hx::IsNull( _this3 )) {
HXLINE( 207)					_hx_tmp4 = 0;
            				}
            				else {
HXLINE( 207)					_hx_tmp4 = _this3->_height;
            				}
HXDLIN( 207)				if ((_hx_tmp3 > _hx_tmp4)) {
HXLINE( 207)					return  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX , ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,l,k,v,r->left,null()),r->key,r->value,r->right,null());
            				}
            				else {
HXLINE( 208)					 ::haxe::ds::TreeNode _hx_tmp5 =  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,l,k,v,r->left->left,null());
HXDLIN( 208)					 ::Dynamic r1 = r->left->key;
HXDLIN( 208)					 ::Dynamic r2 = r->left->value;
HXDLIN( 208)					return  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,_hx_tmp5,r1,r2, ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,r->left->right,r->key,r->value,r->right,null()),null());
            				}
            			}
            			else {
HXLINE( 210)				int _hx_tmp6;
HXDLIN( 210)				if ((hl > hr)) {
HXLINE( 210)					_hx_tmp6 = hl;
            				}
            				else {
HXLINE( 210)					_hx_tmp6 = hr;
            				}
HXDLIN( 210)				return  ::haxe::ds::TreeNode_obj::__alloc( HX_CTX ,l,k,v,r,(_hx_tmp6 + 1));
            			}
            		}
HXLINE( 203)		return null();
            	}


HX_DEFINE_DYNAMIC_FUNC4(BalancedTree_obj,balance,return )

int BalancedTree_obj::compare( ::Dynamic k1, ::Dynamic k2){
            	HX_STACKFRAME(&_hx_pos_97671fefbd3e0550_215_compare)
HXDLIN( 215)		return ::Reflect_obj::compare(k1,k2);
            	}


HX_DEFINE_DYNAMIC_FUNC2(BalancedTree_obj,compare,return )


hx::ObjectPtr< BalancedTree_obj > BalancedTree_obj::__new() {
	hx::ObjectPtr< BalancedTree_obj > __this = new BalancedTree_obj();
	__this->__construct();
	return __this;
}

hx::ObjectPtr< BalancedTree_obj > BalancedTree_obj::__alloc(hx::Ctx *_hx_ctx) {
	BalancedTree_obj *__this = (BalancedTree_obj*)(hx::Ctx::alloc(_hx_ctx, sizeof(BalancedTree_obj), true, "haxe.ds.BalancedTree"));
	*(void **)__this = BalancedTree_obj::_hx_vtable;
	__this->__construct();
	return __this;
}

BalancedTree_obj::BalancedTree_obj()
{
}

void BalancedTree_obj::__Mark(HX_MARK_PARAMS)
{
	HX_MARK_BEGIN_CLASS(BalancedTree);
	HX_MARK_MEMBER_NAME(root,"root");
	HX_MARK_END_CLASS();
}

void BalancedTree_obj::__Visit(HX_VISIT_PARAMS)
{
	HX_VISIT_MEMBER_NAME(root,"root");
}

hx::Val BalancedTree_obj::__Field(const ::String &inName,hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 3:
		if (HX_FIELD_EQ(inName,"set") ) { return hx::Val( set_dyn() ); }
		if (HX_FIELD_EQ(inName,"get") ) { return hx::Val( get_dyn() ); }
		break;
	case 4:
		if (HX_FIELD_EQ(inName,"root") ) { return hx::Val( root ); }
		break;
	case 5:
		if (HX_FIELD_EQ(inName,"merge") ) { return hx::Val( merge_dyn() ); }
		break;
	case 6:
		if (HX_FIELD_EQ(inName,"remove") ) { return hx::Val( remove_dyn() ); }
		if (HX_FIELD_EQ(inName,"exists") ) { return hx::Val( exists_dyn() ); }
		break;
	case 7:
		if (HX_FIELD_EQ(inName,"setLoop") ) { return hx::Val( setLoop_dyn() ); }
		if (HX_FIELD_EQ(inName,"balance") ) { return hx::Val( balance_dyn() ); }
		if (HX_FIELD_EQ(inName,"compare") ) { return hx::Val( compare_dyn() ); }
		break;
	case 10:
		if (HX_FIELD_EQ(inName,"removeLoop") ) { return hx::Val( removeLoop_dyn() ); }
		if (HX_FIELD_EQ(inName,"minBinding") ) { return hx::Val( minBinding_dyn() ); }
		break;
	case 16:
		if (HX_FIELD_EQ(inName,"removeMinBinding") ) { return hx::Val( removeMinBinding_dyn() ); }
	}
	return super::__Field(inName,inCallProp);
}

hx::Val BalancedTree_obj::__SetField(const ::String &inName,const hx::Val &inValue,hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 4:
		if (HX_FIELD_EQ(inName,"root") ) { root=inValue.Cast<  ::haxe::ds::TreeNode >(); return inValue; }
	}
	return super::__SetField(inName,inValue,inCallProp);
}

void BalancedTree_obj::__GetFields(Array< ::String> &outFields)
{
	outFields->push(HX_("root",22,ee,ae,4b));
	super::__GetFields(outFields);
};

#ifdef HXCPP_SCRIPTABLE
static hx::StorageInfo BalancedTree_obj_sMemberStorageInfo[] = {
	{hx::fsObject /*  ::haxe::ds::TreeNode */ ,(int)offsetof(BalancedTree_obj,root),HX_("root",22,ee,ae,4b)},
	{ hx::fsUnknown, 0, null()}
};
static hx::StaticInfo *BalancedTree_obj_sStaticStorageInfo = 0;
#endif

static ::String BalancedTree_obj_sMemberFields[] = {
	HX_("root",22,ee,ae,4b),
	HX_("set",a2,9b,57,00),
	HX_("get",96,80,4e,00),
	HX_("remove",44,9c,88,04),
	HX_("exists",dc,1d,e0,bf),
	HX_("setLoop",26,e7,3b,0d),
	HX_("removeLoop",c8,e0,d6,c4),
	HX_("merge",b8,a2,c6,05),
	HX_("minBinding",d3,d7,e4,44),
	HX_("removeMinBinding",37,f5,8d,fe),
	HX_("balance",1c,9d,8d,23),
	HX_("compare",a5,18,69,83),
	::String(null()) };

hx::Class BalancedTree_obj::__mClass;

void BalancedTree_obj::__register()
{
	BalancedTree_obj _hx_dummy;
	BalancedTree_obj::_hx_vtable = *(void **)&_hx_dummy;
	hx::Static(__mClass) = new hx::Class_obj();
	__mClass->mName = HX_("haxe.ds.BalancedTree",5d,9d,3e,70);
	__mClass->mSuper = &super::__SGetClass();
	__mClass->mConstructEmpty = &__CreateEmpty;
	__mClass->mConstructArgs = &__Create;
	__mClass->mGetStaticField = &hx::Class_obj::GetNoStaticField;
	__mClass->mSetStaticField = &hx::Class_obj::SetNoStaticField;
	__mClass->mStatics = hx::Class_obj::dupFunctions(0 /* sStaticFields */);
	__mClass->mMembers = hx::Class_obj::dupFunctions(BalancedTree_obj_sMemberFields);
	__mClass->mCanCast = hx::TCanCast< BalancedTree_obj >;
#ifdef HXCPP_SCRIPTABLE
	__mClass->mMemberStorageInfo = BalancedTree_obj_sMemberStorageInfo;
#endif
#ifdef HXCPP_SCRIPTABLE
	__mClass->mStaticStorageInfo = BalancedTree_obj_sStaticStorageInfo;
#endif
	hx::_hx_RegisterClass(__mClass->mName, __mClass);
}

} // end namespace haxe
} // end namespace ds
